...
object Main {
  def main(args: Array[String]) {
    ...
    //val newsDataframe: DataFrame = spark.read.json("./data-news-json/")
    ...
    // Tips: use a groupBy
    val newsCountsByCategory = newsDatasets.groupBy("containsWordGlobalWarming").count()
    newsCountsByCategory.show()
    ...
    val resultTyped = spark.sql("SELECT * FROM news WHERE containsWordGlobalWarming = true").as[News]
    resultTyped.show()
