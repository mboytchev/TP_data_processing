...
object ClimateService {
   ...
   * IPCC
   * climate change
   * @param description "my awesome sentence contains a key word like climate change, IPPC and global warming"
   * @return Boolean True
   */
  ...
  def getMinMax(list: List[CO2Record]): (Double, Double) = {
    ...
    val minPpm = ppmValues.min
    val maxPpm = ppmValues.max
  ...
  def getMinMaxByYear(list: List[CO2Record], year: Int): (Double, Double) = {
    ...
    if (recordsForYear.isEmpty) {
      (Double.NaN, Double.NaN)
    } else {
      ClimateService.getMinMax(recordsForYear)
    }
  }
  def getDifferenceMinMax(values: List[Double]): Option[Double] = {
    if (values.isEmpty) {
      None
    } else {
      val min = values.min
